	 	...
	 	for (int iClient = 1; iClient <= MaxClients; iClient++)
		{
			if(IsValidClient(iClient))
    		{
    			...
    			for(int i = 1; i <= MaxClients; i++)
    			{
    				if(IsValidClient(i))
    				{
    					if(JBPlayer(i).bIsWarden)	iWarden = i;
    					if(JBPlayer(i).bIsFreeday)	iFreeday++;
    				}
    			}
			if(iWarden != -1)	PrintHintText(iClient, "Warden: %N \n Guard: %i/%i  Prisoner: %i/%i \n Freeday: %i", iWarden, AliveBlu, Blu, AliveRed, Red, iFreeday);
			else	PrintHintText(iClient, "Warden:  -  \n Guard: %i/%i  Prisoner: %i/%i \n Freeday: %i",AliveBlu, Blu, AliveRed, Red, iFreeday);
